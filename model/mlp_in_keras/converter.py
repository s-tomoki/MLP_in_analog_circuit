...
class Converter:
    ...
    def pooling_4x4(self, X_train, X_test):
        ...
        """
        feature_vector_length = 49
        X_train_pooled = np.array([self._average_pool_4x4(image) for image in X_train]).reshape(
            X_train.shape[0], feature_vector_length
        )
        X_test_pooled = np.array([self._average_pool_4x4(image) for image in X_test]).reshape(
