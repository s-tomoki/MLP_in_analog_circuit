...
import numpy as np
import pytest
from nn_core import Layer, sigmoid_derivative
...
def test_backward_returns_sum_over_neurons():
    ...
    W = np.array([[0.2, -0.4, 0.1], [0.5, 0.3, -0.2]])
    b = np.array([0.1, -0.1])
    errors = np.array([0.6, -0.3])
    layer = Layer(2, 3, rng=np.random.default_rng(0))
    layer.set_weights(W, b)
    layer.forward([1.0, 0.5, -1.0])

    back = layer.backward(errors)

    deltas = np.array(
        [e * sigmoid_derivative(neuron.u) for e, neuron in zip(errors, layer.neurons)]
    )
    assert np.shape(back) == (3,)
    assert back == pytest.approx(W.T @ deltas, rel=1e-12, abs=1e-12)
...
def test_backward_sets_gradients_of_every_neuron():
    ...
    for neuron in layer.neurons:
        assert np.any(neuron.grad_weights != 0)
