...
import numpy as np
import pytest
from nn_core import Neuron, sigmoid, sigmoid_derivative
...
def test_backward_gradients_match_hand_calculation():
    ...
    neuron.backward(0.8)
    assert neuron.grad_weights == pytest.approx([0.2, 0.4], abs=1e-12)
    assert neuron.grad_bias == pytest.approx(0.2, abs=1e-12)
...
def test_backward_returns_error_for_inputs():
    ...
    neuron = make_neuron()
    neuron.forward([1.0, 2.0])
    back = neuron.backward(0.8)
    assert back == pytest.approx([0.1, -0.06], abs=1e-12)
...
def test_backward_does_not_change_weights():
    ...
    neuron.forward([1.0, 2.0])
    neuron.backward(1.0)
    np.testing.assert_array_equal(neuron.weights, [0.5, -0.3])
...
def test_backward_relu_inactive_gives_zero_gradient():
    ...
    neuron = make_neuron("relu")
    neuron.forward([0.0, 1.0])
    back = neuron.backward(1.0)
    assert neuron.grad_weights == pytest.approx([0.0, 0.0], abs=1e-12)
    assert neuron.grad_bias == pytest.approx(0.0, abs=1e-12)
    assert back == pytest.approx([0.0, 0.0], abs=1e-12)
...
def test_backward_step_uses_sigmoid_derivative():
    ...
    neuron = make_neuron("step")
    out = neuron.forward([1.0, 1.0])
    assert out == 1.0
    neuron.backward(1.0)
    d = float(sigmoid_derivative(0.3))
    assert neuron.grad_weights == pytest.approx([d, d], rel=1e-12)
    assert neuron.grad_bias == pytest.approx(d, rel=1e-12)
