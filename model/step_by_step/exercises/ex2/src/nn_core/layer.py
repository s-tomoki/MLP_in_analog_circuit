...
class Layer:
    ...
    def backward(self, errors) -> np.ndarray:
        ...
        """
        # ヒント【段階 1】:
        # - errors の i 番目を、i 番目のニューロンの backward に渡す
        # - 各ニューロンの backward の戻り値(長さ num_inputs の配列)をすべて足し合わせて返す
        #   (前の層の各出力は、この層の全ニューロンに入力されているので、影響を合計する)
        # - 足し合わせる入れ物は、長さ num_inputs のゼロ配列から始めるとよい
